    ...
    return (
        <div className="relative ml-10 py-4">
            ...
            <div className="flex flex-1 flex-col justify-start gap-1">
                <Dates to={entity?.content?.validity?.issuedDate} from={entity?.content?.validity?.expiryDate} />
                <h2 className="font-semibold leading-none">{entity?.title}</h2>
                {entity?.content?.issuer && (
                    <p className="text-sm text-muted-foreground">{entity?.content?.issuer}</p>
                )}
