  ...
  return (
    <li className="relative ml-10 py-4">
      ...
      <div className="flex flex-1 flex-col justify-start gap-1">
        ...
        <MagicRichText className="text-xs" data={research?.description} params={params} searchParams={searchParams} />
      </div>
      {research?.resources && research?.resources.length > 0 && (
        <div className="mt-2 flex flex-row flex-wrap items-start gap-2">
          {research?.resources?.map((link, idx) => (
            // TODO: make a separate ShelfLink component to manage link
