import { MigrateUpArgs, MigrateDownArgs, sql } from '@payloadcms/db-postgres'

export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   ALTER TABLE "achievements" ADD COLUMN "deleted_at" timestamp(3) with time zone;
  ALTER TABLE "_achievements_v" ADD COLUMN "version_deleted_at" timestamp(3) with time zone;
  CREATE INDEX "achievements_deleted_at_idx" ON "achievements" USING btree ("deleted_at");
  CREATE INDEX "_achievements_v_version_version_deleted_at_idx" ON "_achievements_v" USING btree ("version_deleted_at");`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
   DROP INDEX "achievements_deleted_at_idx";
  DROP INDEX "_achievements_v_version_version_deleted_at_idx";
  ALTER TABLE "achievements" DROP COLUMN "deleted_at";
  ALTER TABLE "_achievements_v" DROP COLUMN "version_deleted_at";`)
}
